...
public class DemoDataGenerator implements ModelDataJsonConstants {
  ...
  protected void initProcessDefinitions() {
    ...
    if (deploymentList == null || deploymentList.size() == 0) {
    	deployment = repositoryService.createDeployment().name(deploymentName).addClasspathResource("com/activititest/jpa/example/DummyJPAProcess.bpmn20.xml").deploy();
    }
  ...
  protected void initModelData() {
    createModelData("Demo model", "This is a demo model", "com/activititest/jpa/example/test.model.json");
  }
  ...
  protected void createModelData(String name, String description, String jsonFile) {
    ...
    if (modelList == null || modelList.size() == 0) {
      ...
      try {
        InputStream svgStream = this.getClass().getClassLoader().getResourceAsStream("com/activititest/jpa/example/test.svg");
